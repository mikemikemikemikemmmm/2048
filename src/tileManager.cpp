...
void TileManager::generateNewTile() {
	...
	if (c>16) {
		LOG("container item num");
		LOG(c);
	}
#endif
}

void TileManager::drawAllTiles() {
...
void TileManager::handleDeleteTileObj() {
...
void TileManager::setNewTileMatrixByPressUp() {
	for (int x = 0; x < colLen; ++x) {
		int mergeOtherY = -1;
		//merge
		for (int y = 0; y < rowLen; ++y) {
...
void TileManager::setNewTileMatrixByPressRight() {
	for (int y = 0; y < rowLen; ++y) {
		int mergeOtherX = -1;
		//merge
...
void TileManager::setNewTileMatrixByPressDown() {
	for (int x = 0; x < colLen; ++x) {
		int mergeOtherY = -1;
		//merge
...
void TileManager::setNewTileMatrixByPressLeft() {
	for (int y = 0; y < rowLen; ++y) {
		int mergeOtherX = -1;
		//merge
		for (int x = 0; x < colLen; ++x) {
...
void TileManager::swapTilePtrByIndex(const MatrixIndex& i1, const MatrixIndex& i2) {
...
const bool TileManager::testIsSingleTileGameOverByBPS(const int& x, const int& y) const {
